...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate("/verify"); // ✅ Smooth navigation
    } catch (err) {
      ...
      <div className="lg:min-w-100 bg-white rounded-lg shadow-lg border border-gray-400 m-4 p-1">
        <div className="p-6 space-y-4 sm:p-8">
          ...
          <form className="flex flex-col" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              className={`my-4 w-full text-white font-medium rounded-lg text-sm px-5 py-2.5 ${
                loading ? "bg-gray-400 cursor-not-allowed" : "bg-[#101450]"
              }`}
              disabled={loading}
            >
            ...
            <p className="text-sm font-light text-black flex justify-center border-t pt-2">
              ...
              <Link
                to="/"
                className="font-medium hover:underline text-[#101450] ml-1"
              >
                Login
              </Link>
            </p>
